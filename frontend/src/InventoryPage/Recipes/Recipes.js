...
const fetchRecipesData = (data) => {
    let recipesApi = "https://api.spoonacular.com/recipes/findByIngredients?ingredients=" + data.map((item,key)=>{
        if (key == 0) {
...
return (
    <div>
        <h1>Recipes</h1>
        <ul>
            {!inventory.length?<div>There is no recipe yet</div> : (loading? <div>Loading...</div> :recipesData.map((item,key)=><Recipe item={item} key={key}/>))}
        </ul>
    </div>
  )
}
